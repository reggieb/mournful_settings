...
Note that existing encrypted settings will break if the cipher is changed.
Decrypt existing settings (save with :encrypted => false), change the cipher,
and then re-encrypt the settings (save with :encrypted => true). 
